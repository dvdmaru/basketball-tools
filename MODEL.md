...
| `articles/clippers-kawhi-leonard-circumvention-penalties/` 的「快艇還能循哪一條路走，現有資料不足以判定」與「調查仍在持續接收資訊」 | 下列任一發生時：①快艇實際提起訴訟或其他程序 ②聯盟依 `will consider further action as appropriate` 採取進一步行動 ③官方或工會公開說明那份「final and binding」協議的性質 | 這個狀態**刻意只集中在 2 個地方**：第 11 節（H2「快艇說要挑戰，聯盟說已經定案」整節）與 FAQ「這件事還有得吵嗎？」。⭐ **標題與封面副標刻意不承載這個狀態**（同柯瑞篇的理由：進行中狀態載不動限定詞），所以**不必重產 PNG**；照湖人篇 pattern 在開頭補「更新」引文區塊即可。⚠️ 改的時候**不可放寬既有限定**：CBA 第三十二條只規定了誰能發動**系統仲裁**（只有聯盟與球員工會），管不到法院——⛔ 永遠不要寫成「快艇沒有救濟途徑」或「快艇不能上訴」 |
...
10. ☠️ **把條文／規則白話化時，最先蒸發的是「二擇一／取較晚者」這種限定**（2026-09-01 柯瑞文上線後稽核）。
    CBA §3(a)(2)(i) 原文是「第 4 個薪資上限年度**或**第一個始於球員滿 38 歲後的球季，**兩者取較晚者**」；
    正文與 FAQ 都白話成「落在 38 歲之後那幾季的薪資往前攤」——限定不見了，
    **而且與稿件自己後文的「第 4 季起」互相矛盾**。
    ☠️ **兩席查核都沒抓到，是被額度砍掉的那一席補跑後才發現，當時文章已經上線。**
    ⇒ 凡引用條文／規則的句子，回原文逐字比對**每一個連接詞與限定子句**
    （`or`／`whichever is later`／`unless`／`except`／`no earlier than`），
    白話版要能還原成原文的每一個分支。
    ⇒ 便宜的自檢：**同一份稿件裡對同一機制的兩處敘述並排讀**——本例的自相矛盾本身就是漏限定的指紋。
11. ☠️ **英文引文抄對 ≠ 中文釋義對；釋義要單獨當一道查核**（同輪）。
    ESPN 原句 `will be 38 or older`（未來式）被譯成「已滿或超過 38 歲」，
    而當事人在該報導刊出當天仍是 37 歲。**英文引文逐字無誤，錯的是緊接其後的中文括號釋義**——
    但中文讀者拿到的正是釋義。
    ⇒ 驗收要拆成兩道：①引文與原文逐字比對 ②釋義與原文**語意**比對（特別是時態、情態、單複數）。
...
- **快照欄位增減 → 同一個 PR 更新 §1 的表。**
  ...
  就是紅燈，錯誤訊息會指回本檔。**快照 vs 測試集合的漂移由 CI 抓；MODEL.md 表格 vs 測試集合仍靠同一 PR 人工同步**（測試對本檔只驗存在與三個錨字，不比對表格）。
- 新增聯盟／新增人工維護欄位／新增紅線 → 同 PR 更新本檔。
- ⛔ 其餘一律不要動本檔。改得愈頻繁，愈沒有人信它。

### ☠️ 測試會不會偷寫版控產物：判準是「測試跑不跑生成器」（2026-09-03/04 racing 實例）

racing 的測試套件**真的會跑全量重生**，於是三個測試模組把輸出目錄逐一列名重導到 tmp 時漏了一個
owner，每跑一次全套測試就把 79 個頁面寫進版控裡的產物目錄——而且是**測試合成資料庫**產生的
（一度把某車手寫成某年共同優勝者、勝場 +1，並被 commit 進去）。

⚠️ **本站目前沒有這個問題，但原因不是架構不同，是觸發條件還沒出現**：實測跑全套（50 tests
／0.06 秒）前後，`public-basketball/` 等版控輸出零改寫、零新增、零新建。**秒級＝測試沒在跑
生成器**。一旦有人加一個「端到端跑 gen-*／build-* 再驗輸出」的測試（那是很自然的下一步），
本站就具備完全相同的條件：有 7 支生成器、有 46 個產物進版控。
...
⇒ 加那種測試時，**輸出目錄一律從單一常數重導，不要在測試裡逐一列名**；並記得
**輸出根目錄可能不只一個**（racing 就有兩個：頁面在 PUB、sitemap part 在 `data/` 底下，
後者不在 PUB 之內所以重導 PUB 導不走它）。

### ⭐ 每道 gate 分別擋什麼，要用注入缺陷實測，不能用推論

racing 為這件事寫了 5 道互補的測試，逐一注入缺陷確認各自會咬。最值得記的一條結果：
**把重導拿掉之後，「跑完測試後產物零改動」那道仍然全綠**——因為該寫入端內容相同就跳過、
mtime 不動。**mtime 型斷言對這類寫入端是盲的**，只有「攔寫入目標」那道抓得到。
⇒ 寫完 gate 之後**必須逐一注入它宣稱擋的缺陷**；注入後仍綠的 gate 要移除，不要出貨——
留著會讓下一個人以為這層有守。
